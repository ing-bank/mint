...
/** An example action abstraction mechanism.
 ...
 */
object ExampleActionAbstraction : AndroidConstants {
    fun mapper(): (Node) -> Node? = { x ->
        if (x is Element) {
            if (x.hasAttribute("resourceName") && x.namespaceURI == null) {
                val doc = x.ownerDocument
                val e = doc.createElementNS(AndroidConstants.ABSTRACT_ACTION_NS, "state")
